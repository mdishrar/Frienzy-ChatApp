...
const AudioCall = () => {
    ...
    const isCallerRoleRef = useRef(isCallerRole)
    const remotePeerIdRef = useRef(remotePeerId)
    const remoteUserRef   = useRef(remoteUser)

    useEffect(() => {
        isCallerRoleRef.current = isCallerRole
        remotePeerIdRef.current = remotePeerId
        remoteUserRef.current = remoteUser
    }, [isCallerRole, remotePeerId, remoteUser])

    const startTimer = () => {
        if (timerRef.current) return   // guard against double-starting the timer
    ...
    const startCallerFlow = async () => {
        try {
            ...
            socket.emit('call:offer', {
                to: remotePeerIdRef.current,   // ✅ selectedUser._id
                from: authUser._id,
                offer
            })
    ...
    useEffect(() => {
        const handleOffer = ({ offer }) => {
            if (!isCallerRoleRef.current) startCalleeFlow(offer)
        }
        ...
        const handleAnswer = async ({ answer }) => {
            try {
                if (pcRef.current) {
                    ...
                    remoteDescSetRef.current = true
                    await flushPendingCandidates()   // ✅ drain any candidates that arrived early
                    setCallStatus('Connected')
                    startTimer()
                }
            } catch (err) {
        ...
        const handleCallEnded = () => {
            toast(`Call ended by ${remoteUserRef.current?.fullName || 'other user'}`)
            cleanup()
            navigate('/')
        }
